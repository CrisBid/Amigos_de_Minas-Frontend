...
export async function GET(request: Request, ctx: Ctx) {
  ...
  const r = await fetch(`${API}/sponsorships/${id}`, { headers: { Authorization: `Bearer ${at}` }, cache: 'no-store' });
...
export async function PATCH(request: Request, ctx: Ctx) {
  ...
  const r = await fetch(`${API}/sponsorships/${id}`, {
    method: 'PATCH',
...
export async function DELETE(request: Request, ctx: Ctx) {
  const at = await requireAccessToken(request);
  const { id } = await ctx.params;
  const r = await fetch(`${API}/sponsorships/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${at}` } });
  return new Response(await r.text(), { status: r.status, headers: { 'content-type': r.headers.get('content-type') ?? 'application/json' } });
}
