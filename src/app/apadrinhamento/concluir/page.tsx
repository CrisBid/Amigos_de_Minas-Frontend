...
export default function ConcluirApadrinhamentoPage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.replace(`/apadrinhamento/registro?childId=${encodeURIComponent(childId)}&campaignId=${encodeURIComponent(campaignId)}`);
    }
  }, [status, childId, campaignId, router]);

  async function confirm() {
    if (!api || !accessToken) { setError('Sessão inválida.'); return; }
    ...
    try {
      ...
      setError(e?.message || 'Não foi possível concluir o apadrinhamento.');
    } finally {
      setSubmitting(false);
    }
  }
